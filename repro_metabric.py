...
def train_deephit(train_set, val_set, labtrans):
	...
	return log['train_loss'], log['val_loss'], model
...
train_loss, val_loss, deephit_model = train_deephit(train_set, val_set, STConfig['labtrans'])
plot_train_val_loss(train_loss, val_loss, 'DeepHit on metabric')
# evaluation for deephit
deephit_model.config = STConfig
surv = deephit_model.interpolate(10).predict_surv_df(df_test.to_numpy(np.float32))
evaluator = EvalSurv(surv, df_y_test['duration'].values, df_y_test['event'].values)
ev.concordance_td('antolini')
durations_test = df_y_test['duration'].values
time_grid = np.linspace(durations_test.min(), durations_test.max(), 100)
ev.brier_score(time_grid).plot()
plt.ylabel('Brier score')
_ = plt.xlabel('Time')

# train_loss, val_loss, survTrace_model = train_survTrace(STConfig, train_set, val_set)
# plot_train_val_loss(train_loss, val_loss, 'SurvTRACE on metabric')
evaluator = Evaluator(df, df_train.index)
evaluator.eval(deephit_model, (df_test, df_y_test))
